...
test('bigrams beat scattered unigrams', () => {
  ...
  const scattered = scoreAgainst(
    'biggest team you led and a weakness of that plan',
    targets[0],
    df,
    docCount
  )
  ...
  cards: [
    {
      ...
      script:
        'I see it less as switching away from development and more as bringing my development background into a different kind of contribution.',
      triggers: [
...
test('a commit at an unlatched final still passes through', () => {
  ...
  assert.deepEqual(
    out.commands,
    cmds,
    'no card matched; the draft is the answer and must be adopted'
  )
...
test('an interim reset that clears a standing latch reports latchCleared', () => {
  ...
  assert.equal(
    out.latchCleared,
    true,
    'the interim path has no other way to learn the card came down'
  )
})

test('latchCleared is false when no latch was standing', () => {
  const state = newLatchState()
  const out = gateCommands([{ kind: 'reset' }], state, {
    suppress: false,
    latch: null,
    isFinal: false
  })
...
test('latchCleared is false when a final replaces one latch with another', () => {
  ...
  assert.equal(
    out.latchCleared,
    false,
    'a card is still standing; the final path renders the new one'
  )
})
...
test('suppression at the final never drops the endpoint fire', () => {
  ...
  assert.ok(
    lastCommand.some((c) => c.kind === 'fire'),
    'scheduler should emit fire after stable interim'
  )

  // Suppress the speculative fire mid-question
  const suppressed = gateCommands(lastCommand, state, {
    suppress: true,
    latch: null,
    isFinal: false
  })
  ...
  assert.ok(
    gated.commands.length > 0,
    'endpoint fire must reach the pipeline to prevent blank screen'
  )
  assert.ok(
    gated.commands.some((c) => c.kind === 'fire'),
    'endpoint fire must pass through'
  )
...
test('a final with no latch still lets the endpoint fire through (recovery must not regress)', () => {
  ...
  assert.ok(
    out.commands.some((c) => c.kind === 'fire'),
    'no card matched; the endpoint fire is the only recovery and must pass through'
  )
  assert.equal(out.resetScheduler, false)
})
...
test("a second question that matches nothing clears the first question's latch, so its regenerate is not dropped", () => {
  ...
  assert.equal(
    state.cardId,
    null,
    'a non-matching final must clear the stale latch from the previous question'
  )
  assert.ok(
    out.commands.some((c) => c.kind === 'regenerate'),
    'Q2 must get its regenerate, not be silently dropped by a stale latch'
  )
})
...
test('inversion: negation in script omitted by cue is rejected', () => {
  ...
  assert.deepEqual(
    verifyCard(card, 'I did not cut costs.').cues,
    [],
    'cue omitting negation must be dropped'
  )
})
...
test('scrambling: tokens in different order are rejected', () => {
  ...
  assert.deepEqual(
    verifyCard(card, 'I cut costs and grew the team.').cues,
    [],
    'reordered tokens must be dropped'
  )
})
...
test('faithful: order-preserving compression is accepted', () => {
  ...
  assert.deepEqual(
    result.cues,
    ['Shipped lead scoring engine'],
    'order-preserving cue must be kept'
  )
})
...
test('negation parity: cue carrying all negations in span is accepted', () => {
  ...
  assert.equal(
    result.cues.length,
    1,
    'cue with negation parity must be kept (did is stopword, cue has [not, cut, costs])'
  )
})
...
test('inflection: a cue may use a different form of a script word', () => {
  ...
  assert.equal(
    keeps(
      'I stop the bleeding before root-causing anything.',
...
test('connectives: a bounded number of unsourced glue words is allowed', () => {
  assert.equal(
    keeps(
      'I split reads onto replicas and moved the write path onto a queue.',
